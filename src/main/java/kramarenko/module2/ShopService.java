...
public class ShopService {
    ...
    public static List<Product> readCsvFile(String fileName) {
        ...
        if (resource == null) {
            ...
            int country = 0;
            int price = 0;
            try (BufferedReader reader = new BufferedReader(new FileReader(new File(((URL) resource).toURI())))) {
                String str;
                ...
                while ((str = reader.readLine()) != null) {
                    ...
                    try {
                        if (strings[price].matches("[^0-9]+")) {
                            throw new IncorrectStringException("price must be numeric \"" + str + "\"");
                        }
                        if (Arrays.asList(strings).contains("")) {
                            throw new IncorrectStringException("value must be not empty \"" + str + "\"");
                        }
                        list.add(strings);
            ...
            for (String[] strings : list) {
                if (strings[type].equals("Telephone")) {
                    ...
                    television.setDiagonal(Integer.parseInt(strings[diagonal]));
